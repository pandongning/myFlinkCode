...
import flink.mystream.beans.{MergeBean, SensorReading, SensorReadingTwo}
...
object T3_Two_Stream_Interval_Join {
  def main(args: Array[String]): Unit = {
    ...
    //    下面的代码是错误的，因为下面要做两个流的join，则此时不能在两个流里面都存在表示水印的字段，因为此时flink则不知道要使用那个字段
    //    val tableSecond: Table = tableEnvironment.fromDataStream(valueSenond, 'id, 'timestamp.rowtime as 'rt, 'name)
    val tableSecond: Table = tableEnvironment.fromDataStream(valueSenond, 'id, 'timestamp.rowtime as 'rt, 'name)
    ...
    tableEnvironment.sqlQuery(
      s"""
         ...
         |and fisrt.ts >= seocnd.rt and fisrt.ts <= seocnd.rt + INTERVAL '5' second
